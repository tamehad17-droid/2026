...
serve(async (req) => {
  ...
  try {
    ...
    const ADGEM_POSTBACK_KEY = Deno.env.get('ADGEM_POSTBACK_KEY'); // 6b133h6i0674mfcca9bnfaid
    
    // Parse URL parameters
    const url = new URL(req.url);
    const params = {
      appid: url.searchParams.get('appid'),
      userid: url.searchParams.get('userid'), // player_id from AdGem
      offerid: url.searchParams.get('offerid'),
      amount: url.searchParams.get('amount'), // Virtual currency amount
      payout: url.searchParams.get('payout'), // Real USD payout
